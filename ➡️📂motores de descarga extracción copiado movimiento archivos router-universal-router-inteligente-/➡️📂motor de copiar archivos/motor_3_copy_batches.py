...
SCHEMA = "yaiwes.frontend.copy-batches.v1"
SOURCE_DIR = pathlib.Path(os.getenv("SOURCE_DIR", "")).expanduser()
DEST_DIR = pathlib.Path(os.getenv("DEST_DIR", "")).expanduser()
STATE_FILE = pathlib.Path(os.getenv("STATE_FILE", ".motor3-copy-state.json")).expanduser()
...
def load_state(files):
    ...
    return {"schema": SCHEMA, "manifest": files, "completed": {}, "failed": {}, "batches": 0}
...
def copy_verified(row):
    ...
    if sha256_file(tmp) != row["sha256"]:
        tmp.unlink(missing_ok=True); raise RuntimeError("COPY_HASH_MISMATCH:" + row["rel"])
    os.replace(tmp, dst)
...
def main():
    ...
    files = manifest(); state = load_state(files); completed = state.setdefault("completed", {}); batches = 0
    while True:
        ...
        for row in pending[:BATCH_SIZE]:
            try:
                status = copy_verified(row); completed[row["rel"]] = status; state.setdefault("failed", {}).pop(row["rel"], None)
    ...
    result = {
        ...
        "total": total, "copied_or_verified": done, "failed": failed, "pending": pending_count,
        "batches_total": state.get("batches", 0), "state_file": str(STATE_FILE),
        "verdict": "VERIFIED_CLOSED" if pending_count == 0 and failed == 0 else "GAPS_PENDING"
    }
